...
void findSimilar()//可以用二维指针数组进行优化，一共是0123四行，每一行按顺序存汉明距离为行号的网页ID
{
    ...
    for(int i=0;i<page2;i++) {
        int pos0 = 0, pos1 = 0, pos2 = 0, pos3 = 0,j;
        fprintf(dst, "%s\n", sample[i].webID);
        for (j = 0; j < page1; j++) {
            if (HammingDistance(sample[i].fingerprint, article[j].fingerprint) == 0) {
                if (pos0 == 0) {
                    fprintf(dst, "0:%s ", article[j].webID);
                    pos0++;
                } else {
                    fprintf(dst, "%s ", article[j].webID);
                }
            }
        }
        if(pos0>0)fputc('\n',dst);
        for (j = 0; j < page1; j++) {
            if (HammingDistance(sample[i].fingerprint, article[j].fingerprint) == 1) {
                if (pos1 == 0) {
                    fprintf(dst, "1:%s ", article[j].webID);
                    pos1++;
                } else {
                    fprintf(dst, "%s ", article[j].webID);
                }
        ...
        if(pos1>0)fputc('\n',dst);
        for (j = 0; j < page1; j++) {
                if (HammingDistance(sample[i].fingerprint, article[j].fingerprint) == 2) {
                    if (pos2 == 0) {
                        fprintf(dst, "2:%s ", article[j].webID);
                        pos2++;
                    } else {
                        fprintf(dst, "%s ", article[j].webID);
                    }
        ...
        if(pos2>0)fputc('\n',dst);
        for (j = 0; j < page1; j++) {
                if (HammingDistance(sample[i].fingerprint, article[j].fingerprint) == 3) {
                    if (pos3 == 0) {
                        fprintf(dst, "3:%s ", article[j].webID);
                        pos3++;
                    } else {
                        fprintf(dst, "%s ", article[j].webID);
                    }
