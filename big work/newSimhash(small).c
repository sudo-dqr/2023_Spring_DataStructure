...
Stopwordptr STOPWORD[1000]={NULL};
typedef struct wordlist{
    char word[100];
    int weight;
    struct wordlist*link;
}Wordlist,*Wordlistptr;
Wordlistptr WORDLIST[1000000]={NULL};
struct web{
    char webID[20];
    char fingerprint[200];
};
struct web article[100000];
struct web sample[10000];
...
void StopwordHASH()
{
    ...
    for( i=0;buffer[i];i++){
        ...
        unsigned  int hash= SDBMHash(word,1000);
...
{
    unsigned int tmp= SDBMHash(w,1000);
...
Wordlistptr ptr2[106000]={NULL};
int ptr2cnt=0;
void VectorWordHASH()//关键单词HASH表
{
    for(int i=0;i<N;i++){//利用指针数组建立hash表
        unsigned int hash= SDBMHash(ptr[i]->word,100000);
        Wordlistptr p=(Wordlistptr)malloc(sizeof(Wordlist));
...
{
    unsigned int hash= SDBMHash(w,100000);
...
int page1=0,page2=0;
int GetWord(FILE*fp,char *word,struct web* a,int *pagen,int weightn[10000][10000])
{
    int ch;
...
int weight1[10000][10000];
int weight2[10000][10000];
void GetEachWeb(FILE *fp,struct web*a,int *pagen,int weightn[10000][10000])//对每个网页统计
{
    char word[100];
    memset(word,0,sizeof(word));
...
void FINGERPRINT(int pagen,struct web*a,int weightn[10000][10000])
{
...
struct web* PrintHamming[16000][4][1000];//页码 汉明距离 article个数
void findSimilar()//可以用二维指针数组进行优化，一共是0123四行，每一行按顺序存汉明距离为行号的网页ID
{
    memset(PrintHamming,0,sizeof(PrintHamming));
    FILE *dst=fopen("result.txt","w");
    for(int i=0;i<page2;i++){//对每一个sample找article,一遍遍历即存入
        int pos0=0,pos1=0,pos2=0,pos3=0;
        for(int j=0;j<page1;j++){
            if(HammingDistance(sample[i].fingerprint,article[j].fingerprint)==0)
                PrintHamming[i][0][pos0++]=&article[j];
            else if(HammingDistance(sample[i].fingerprint,article[j].fingerprint)==1)
                PrintHamming[i][1][pos1++]=&article[j];
            else if(HammingDistance(sample[i].fingerprint,article[j].fingerprint)==2)
                PrintHamming[i][2][pos2++]=&article[j];
            else if(HammingDistance(sample[i].fingerprint,article[j].fingerprint)==3)
                PrintHamming[i][3][pos3++]=&article[j];
        }
    }
    for(int i=0;i<page2;i++){
        fprintf(dst,"%s",sample[i].webID);
        fputc('\n',dst);//先打印网页名
        for(int j=0;j<4;j++){
            if(PrintHamming[i][j][0]==NULL)continue;
            else {
                fprintf(dst,"%d:%s ",j,PrintHamming[i][j][0]->webID);
                int k=1;
                while(PrintHamming[i][j][k]!=NULL)
                    fprintf(dst,"%s ",PrintHamming[i][j][k++]->webID);
                fputc('\n',dst);
            }
        }
...
//突然发现词频统计中article少读一页，sample没有少读一页
int main(int argc,char *argv[]) {
    ...
    FINGERPRINT(page2,sample,weight2);
    findSimilar();
    return 0;
}
